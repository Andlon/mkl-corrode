use mkl_corrode::dss::{MatrixDefiniteness, MatrixStructure, SymbolicFactorization};
...
fn dss_symbolic_factorization() {
    ...
    let _ = SymbolicFactorization::<f64>::try_from_csr(
        &row_ptr,
        &columns,
        MatrixStructure::NonSymmetric,
    );
}
...
fn dss_1x1_factorization() {
    ...
    let symb = SymbolicFactorization::try_from_csr(&row_ptr, &columns, MatrixStructure::Symmetric)
        .unwrap();
...
fn dss_factorization() {
    ...
    let symb =
        SymbolicFactorization::try_from_csr(&row_ptr, &columns, MatrixStructure::NonSymmetric)
            .unwrap();
